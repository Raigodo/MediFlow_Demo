...
use App\Http\Resources\Invitation\InvitationPreviewResource;
use App\Http\Resources\Invitation\Base\InvitationsActionsResource;

class InvitationPreviewPagination extends ResourceCollection
{
    ...
    public function toArray(Request $request): array
    {
        $this->paginator->getCollection()
            ->transform(fn($item)
                => (new InvitationPreviewResource($item))
            );
