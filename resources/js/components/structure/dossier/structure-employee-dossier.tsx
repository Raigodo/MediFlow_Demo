...
function StructureEmployeeDossier({ structure }: { structure: StructureDetailResource }) {
    const { collection, filter, ...rest } = usePagniatedEmployees();

    return (
        <ResourceSectionContextProvider defaultSectionKey="employees">
            <DossierLayout actions={{ title: 'Structure', dropdown: structure.actions.dropdown }}>
                ...
                <DossierContentTable pagination={rest} paginationBag={filter}>
                    <EmployeeTable collection={collection} />
                </DossierContentTable>
            </DossierLayout>
...
export default StructureEmployeeDossier;
